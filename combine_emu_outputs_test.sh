...
paste all_unique_taxa.tsv <(cut -f2- combined_emu_counts.tsv) > combined_emu_counts_with_taxonomy_test.tsv

# Step 5: Create CSV version (from final table with taxonomy)
sed 's/\t/,/g' combined_emu_counts_with_taxonomy_test.tsv > combined_emu_counts_with_taxonomy_test.csv
...
echo "Done! Created:"
echo "- combined_emu_counts_with_taxonomy_test.tsv"
echo "- combined_emu_counts_with_taxonomy_test.csv" 
echo "Total unique taxa: $(tail -n +2 combined_emu_counts_with_taxonomy_test.tsv | wc -l)"
echo "Samples included: ${#SAMPLES[@]}"

# Verify the data looks correct
echo ""
echo "First few rows of the combined data:"
head -3 combined_emu_counts_with_taxonomy_test.tsv
